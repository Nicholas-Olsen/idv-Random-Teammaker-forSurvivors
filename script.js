...
applyMapSynergy('작곡가', ['붉은성당', '군수공장', '차이나타운', '레오의기억'], 10); // 신규 시너지 
...
// [신규 조건] 모험가가 픽됐을 때 F조 그룹 가중치 하락
if (picked.name === '모험가') groupWeights.f -= 40; 

// [신규 조건] 특정 맵에서 장난감상인 픽됐을 때 F조 그룹 가중치 급락
...
// 💡 [신규 조건] 캐릭터 이름이 '모험가'일 경우 역할 이름을 특수구출로 덮어씌움
let displayRole = roleNames[member.list];
if (charName === '모험가') {
displayRole = '특수구출';
} 

card.innerHTML = &lt;div class="list-name"&gt;${displayRole}</div>
